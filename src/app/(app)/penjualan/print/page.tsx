...
export default function PrintPenjualanAllPage() {
  ...
  useEffect(() => {
    fetch(`/api/penjualan${q ? `?q=${encodeURIComponent(q)}` : ""}`)
      .then((r) => r.json())
      .then(setData)
      ...
      <div className="print-area card space-y-4">
        <div className="border-b border-slate-200 pb-4">
          ...
          <p className="text-sm text-slate-500">
            Laporan Penjualan Barang {q && <span>— pencarian: &quot;{q}&quot;</span>}
          </p>
              ...
              <tbody>
                {data.map((p) => (
                  <tr key={p.id} className="border-b border-slate-100">
                    ...
                    <td className="py-2 text-slate-600">{p.pelanggan?.nama ?? "Umum"}</td>
